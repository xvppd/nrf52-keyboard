...
{
    if (start_pos >= RGBLED_NUM)
        return;
    if (start_pos + num_leds > RGBLED_NUM)
        return;
 ...
 *
 */
uint32_t eeconfig_read_rgblight(void)  //need mod
{
    return ws2812_rgb.data[0] | (ws2812_rgb.data[1] << 8)
        | (ws2812_rgb.data[2] << 16) | (ws2812_rgb.data[3] << 24);
}
/**@brief 写入ws2812存储数据.
 *
 */
void eeconfig_update_rgblight(uint32_t val)  //need mod
{
    if ((ws2812_rgb.data[3] | (ws2812_rgb.data[2] << 8)
            | (ws2812_rgb.data[1] << 16) | (ws2812_rgb.data[0] << 24))
        != val) {
        ws2812_rgb.data[0] = (val % 0x100);
        ws2812_rgb.data[1] = (val >> 8);
        ws2812_rgb.data[2] = (val >> 16);
        ws2812_rgb.data[3] = (val >> 24);
        storage_write((1 << STORAGE_CONFIG));
    }
}

void eeconfig_update_rgblight_default(void)  //need mod
{
...
void eeconfig_debug_rgblight(void)  //need mod
{
    // do nothing
}

void rgblight_init(void)  //need mod
{
...
void rgblight_reload_from_eeprom(void)    //need mod
{
...
void rgblight_disable(void)  //need mod
{
...
void rgblight_blink_layer_repeat(uint8_t layer, uint16_t duration_ms, uint8_t times)
{
    ...
    _repeat_timer = timer_read() + duration_ms;
}

void rgblight_blink_layer_repeat_helper(void)
{
    if (_blinking_layer_mask != 0 && timer_expired(timer_read(), _repeat_timer)) {
...
//休眠关机时调用关闭RGB
void rgblight_sleep_prepare(void)  //need mod
{
    // 禁用RGB LED
    rgblight_disable_noeeprom();
    wait_ms(1);
...
// Animation timer -- use nrf52 Timer
void rgblight_timer_init(void)  //need mod
{
    rgblight_status.timer_enabled = false;
    RGBLIGHT_SPLIT_SET_CHANGE_TIMER_ENABLE;
}
void rgblight_timer_enable(void) //need mod
{
    if (!is_static_effect(rgblight_config.mode)) {
...
void rgblight_timer_disable(void) //need mod
{
    rgblight_status.timer_enabled = false;
    RGBLIGHT_SPLIT_SET_CHANGE_TIMER_ENABLE;
}
void rgblight_timer_toggle(void) //need mod
{
...
void rgblight_effect_twinkle(animation_status_t* anim)
{
    ...
    uint16_t scale(uint16_t v, uint8_t scale) { return ((uint32_t)v * scale) >> 8; }
    ...
    for (uint8_t i = 0; i < rgblight_ranges.effect_num_leds; i++) {
        ...
        } else if ((uint16_t)rand() < scale((uint16_t)RAND_MAX * RGBLIGHT_EFFECT_TWINKLE_PROBABILITY, 127 + rgblight_config.val / 2)) {
...
static void status_rgblight_evt_handler(enum user_event event, void* arg) //need mod
{
...
EVENT_HANDLER(status_rgblight_evt_handler);  //need mod